...
namespace Exam_Retake
{
    ...
    public class Tests
    {
        ...
        [Test, Order(7)]
        public void DeleteNonExistentRevue_ShouldReturn_BadRequest_Msg()
        {
            var request = new RestRequest($"{baseUrl}/api/Revue/Delete?revueId=nonexistentid", Method.Delete);
            var response = client.Execute(request);
